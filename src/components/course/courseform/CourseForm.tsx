...
import { uploadToCloudinary, uploadVideoToCloudinary } from "../../../utilities/axios/claudinary";
  ...
  const isValidDuration = (duration?: string) => {
    ...
    const validateForm = () => {
      ...
      const allVideosValid = course.lessons?.every(
        (lesson) => lesson.video && isValidDuration(lesson.duration)&&lesson.video.trim().length > 0
      );
      const hasThumbnail = course.thumbnail && course.thumbnail.trim().length > 0;
      const isValid = hasTitle && hasDescription && allVideosValid && hasThumbnail;
      return isValid;
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      let response;
      if(courseDetails){
        response = await dispatch(editCourse({data:{...course,_id:courseDetails._id},studentId:""}))
      }
      else{
        response = await dispatch(addCourse({...course,instructorRef:data?._id,language:"English"}))
      }
    ...
    return (
      <form onSubmit={handleSubmit} className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg">
        ...
        <div className="space-y-6">
          ...
          <ThumbnailUpload onUpload={handleThumbnailUpload} thumbnailPreview={thumbnailPreview}/>
          
          <LessonsSection 
            course={course} 
            setCourse={setCourse} 
            onVideoUpload={(file: File, lessonIndex: number) => handleVideoUpload(file, lessonIndex)}
            progress={progress}
          />
