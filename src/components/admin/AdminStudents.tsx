...
const AdminStudents = () => {
    ...
    // Fetch students data based on current params
            ...
            if (getAllStudents.fulfilled.match(resultAction)) {
                ...
                if (data && Array.isArray(data)) {
                    const verifiedStudents = data.filter(
                        (studentData: SignupFormData) => studentData.isVerified
                    );
                    
                    setStudents(verifiedStudents);
                    setTotalPages(pages || Math.ceil(totalCount / ITEMS_PER_PAGE) || 1);
                    setTotalItems(totalCount || verifiedStudents.length);
                } else {
                    setStudents([]);
                    setTotalPages(1);
                    setTotalItems(0);
                ...
                const errorMessage = errorPayload?.message || "Failed to fetch students";
        ...
        try {
            if (selectedUser) {
                ...
                if (response.payload.success) {
                    ...
                    ToastService.error(
                        `Failed to ${selectedUser?.isBlocked ? "unblock" : "block"} student`
                    );
                }
    ...
    return (
        <div className="p-4">
            ...
            {loading ? (
                ...
                <>
                    <UsersList users={students} onEdit={handleEditClick} />
                ...
                <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                    {currentSearch ? "No students match your search criteria." : "No students found."}
                </div>
            )}
        </div>
    );
